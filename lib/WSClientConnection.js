...
class WSClientConnection extends EventEmitter {
  ...
  constructor(con, { closeTimeout = 2000 } = {}) {
    ...
    this.con.on("close", () => this.state = 3)
  }
  /**
   * @private
   */
   ...
   * @private
   */
  async _close({ code = null, reason = "", unclean = false, recieving = false }) {
    if (unclean) {
      this.state = 3;
      this.emit("close", 1006, "", true);
      return this.con.end()
    }
    if (code == null) {
      await this._send({
  ...
  _handleMessage({ FIN, reserved, opcode, masked, length, maskingKey, payload }) {
    ...
    if (opcode == 0) { // Continuation frame
      ...
      if (FIN) {
        ...
        if (this._opcode == 1) {
          if (!validateUTF8(this._payload)) return this._close({ code: 1007 })
      ...
      this._opcode = 1
      if (FIN) {
        if (!validateUTF8(payload)) return this._close({ code: 1007 })
